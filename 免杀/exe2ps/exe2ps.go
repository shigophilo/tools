...
func main() {
	...
	flag.StringVar(&file, "f", "", "要转换的exe文件")
	flag.StringVar(&name, "n", "", "执行函数的名称")
	flag.StringVar(&out, "o", "out.ps1", "保存文件")
	flag.IntVar(&len, "l", 0, "执行函数的长度")
	flag.Parse()
	if file == "" {
		fmt.Println("-f 不能为空")
		return
	} else {
		base64Str = tobase(file)
	}
	if name == "" && len == 0 {
		randomString = generateRandomString(10)
		tops(base64Str, randomString, out)
		return
	}
	if name != "" && len != 0 {
		fmt.Println("-n 和 -l 参数只能使用一个")
		return
	}
	if name != "" && len == 0 {
		tops(base64Str, name, out)
		return
	}

}

func tops(base64Str string, randomString string, filePath string) {

	ps := `$InputString = '` + base64Str + `'
	$PEBytes = [System.Convert]::FromBase64String($InputString)
	
	function Shi-` + randomString + `
	{
	...
	// 将多行字符串写入文件
	err := ioutil.WriteFile(filePath, []byte(ps), 0644)
	if err != nil {
		log.Fatal(err)
	}
	fmt.Println("使用方法: Import-Module .\\" + filePath + " ; " + "Shi-" + randomString + " -PEBytes $PEBytes")
}
